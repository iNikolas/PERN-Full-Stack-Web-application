...
const Paginator = ({ pagination, setCurrentPage }) => {
  ...
  return (
    <Pagination>
      ...
      {first && prev && (
        <Pagination.Item onClick={() => setCurrentPage(first)}>
          1
        </Pagination.Item>
      )}
      ...
      {prevPage > 1 && (
        <Pagination.Item onClick={() => setCurrentPage(prev)}>
          {prevPage}
        </Pagination.Item>
      )}
      {<Pagination.Item active>{currentPage}</Pagination.Item>}
      {totalPages - currentPage > 1 && (
        <Pagination.Item onClick={() => setCurrentPage(next)}>
          {nextPage}
        </Pagination.Item>
      )}
      ...
      {last && next && (
        <Pagination.Item onClick={() => setCurrentPage(last)}>
          {totalPages}
        </Pagination.Item>
      )}
